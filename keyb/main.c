/* This is a polyphonic key-finding program. It takes input either in
the form of "Note" statements (above) or "TPCNote" statements (below):

Note [ontime] [offtime] [pitch]
TPCNote [ontime] [offtime] [pitch] [TPC]

where ontime and offtime are in milliseconds, pitch is in integer
notation (middle C = 60), and TPC is in "line-of-fifths" pitch-class
notation (Gb=-4...C=2, G=3, D=4...F#=6, etc.).

The program also requires "beat" statements of the form "Beat [time]
...
With verbosity = 1, the program outputs the key analysis, giving a key
symbol for each segment. With verbosity = 2 or 3, it outputs a lot
