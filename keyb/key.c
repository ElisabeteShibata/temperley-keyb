...
/* _Important comment about how pitches and keys are represented_
  ...
  otherwise a fatal error is reported.) In short: Note input with a
  TPC profile is forbidden.

  Some scoring algorithms also require NPC profile, namely the K-S
  algorithm (scoring_mode = 0) and the Bayesian algorithm
  (scoring_mode = 1). If one of these is used with a TPC profile,
  it's a fatal error.
...
int create_chords() {                                   /* Now we create the chords; each chord starts and ends at a beat of
...
int create_segments() {          /* Each segment starts at a sbeat and ends at the following sbeat */
    int b, s;
    ...
    if (segment[s].start>=final_timepoint) s--;
    else {
...
int fill_segments() {
  ...
  for (s=0; s<=segtotal; s++) {
    for (x=0, n=0; x<numnotes; x++) {
      ...
      if (note[x].ontime >= segment[s].start && note[x].ontime < segment[s].end
          && note[x].offtime > segment[s].end) {   /* note begins, doesn't end in segment*/
    	segment[s].snote[n].pitch=note[x].pitch;
    	segment[s].snote[n].tpc=note[x].tpc;
    	segment[s].snote[n].duration = segment[s].end - note[x].ontime;
    	++n;
      }
      if (note[x].ontime < segment[s].start && note[x].offtime > segment[s].start
        && note[x].offtime <= segment[s].end) { /* note ends, doesn't begin in segment */
      	segment[s].snote[n].pitch=note[x].pitch;
      	segment[s].snote[n].tpc=note[x].tpc;
      	segment[s].snote[n].duration = note[x].offtime - segment[s].start;
      	++n;
      }
      if (note[x].ontime < segment[s].start && note[x].offtime > segment[s].end) { /* note doesn't begin or end in segment */
      	segment[s].snote[n].pitch=note[x].pitch;
      	segment[s].snote[n].tpc=note[x].tpc;
...
int count_segment_notes() {                             /* In each segment, tally up the notes of each TPC */
    int s, y, n;
    double total_dur;
...
int match_profiles() {
    ...
    for (s=0; s<=segtotal; s++) {
      ...
      for (key=0; key<56; key++) {
        ...
        /* kprob is the probability of the pitch set given the key (calculated only
        for the local segment). (key_score[s][key] is just the log of this value.)
        First set kprob to 0 for all keys, then (just below)
        set it to 1 for keys to be considered by the Bayesian model.  */
        ...
        for (tpc=0; tpc<28; tpc++) {
          ...
          profile values represent total duration of each pc (in all other cases, they're just 1
  		    ...
  		    represent pc distributions in a corpus, i.e. the proportion of segments containing each scale
  		    degree)
          */
          ...
          if(scoring_mode == 3) {
            /* if(tpc>11) continue; */
            /* if(tpc<9 || tpc>20) continue; */ /* Not needed - scoring_mode=3 requires an NPC profile */
      ...
      if(scoring_mode==3) {
        ...
        if(verbosity>=3) {
          printf("Best key for segment %d = %d, p(pcset | key) = %6.8f, p(pcset & key) = %6.8f\n", s, best_key, kprob[best_key], kprob[best_key] / 24.0);
          printf("Overall local probability of segment %d = P(pcset) = %6.8f\n", s, total_prob[s]);
        }
        /* Now total_prob[s] is the total probability of the segment occurring: its probability in combination with
        a key, summed over all keys. But suppose we want to know the probability of ANY major triad
        occurring? Then we have to multiply this by 12. (Except for something like a diminished
        seventh which is symmetrical!) We don't do this here though. */
      }

      // To see local scores for specific segments:
      if(verbosity>2 && scoring_mode == 0 && s==0) {
        for (key=0; key<56; key++) {
          if(key<9 || (key>20 && key<37) || key>48) continue;
          printf("segment %d, key score for key %d = %6.3f\n", s, key, key_score[s][key]);
        }
      }

      if(verbosity>2 && scoring_mode == 3) {
        printf("P(key | pcset) for all 24 keys (on line of fifths): ");
        for (key=0; key<56; key++) {
          if(key<9 || (key>20 && key<37) || key>48) continue;
          /* In the statement below: p(key | seg) = p (key & seg) / p (seg)
          = p (seg | key) p(key) / p(seg)
          = (exp(key_score[s][key]) * 1/24) / total_prob[s] */

          //if(s==0 || s==4) printf("segment %d local score for key %d: %6.3f (%6.8f); P(key | seg) = %6.3f\n", s, key, key_score[s][key], exp(key_score[s][key]), exp(key_score[s][key]) / (24.0 * total_prob[s]));
          printf("%6.3f ", exp(key_score[s][key]) / (24.0 * total_prob[s]));
          pkeyseg[key] = exp(key_score[s][key]) / (24.0 * total_prob[s]);
          //if(s==0) printf("p(seg | key=%d) = %6.4f\n", key, exp(key_score[s][key]));
        }
        printf("\n");
        /* for (key=9; key<=20; key++) {
        printf("seg %d combined score for parallel keys %d and %d: %6.8f; P(kM or km | seg) = %6.3f\n", s, key, key+28, exp(key_score[s][key]) + exp(key_score[s][key+28]), ((exp(key_score[s][key])/24.0) + (exp(key_score[s][key+28])/24.0)) / total_prob[s]);
        } */
        ...
        for(key=0; key<56; key++) {
          if(key<9 || (key>20 && key<37) || key>48) continue;
      		if(pkeyseg[key] > best_score) {
            best_score = pkeyseg[key];
            best = key;
          }
        }
        ...
        if(best !=0 && second_best !=0) {
          if(pkeyseg[second_best] == 0.0) {
            printf("Ambiguity score = infinite, because second-best key has zero score\n\n");
      		}
      		else printf("Best key = %d (score = %6.3f), second best = %d (score = %6.3f): ambiguity score = %6.3f\n\n",
                      best, pkeyseg[best], second_best, pkeyseg[second_best], pkeyseg[best] / pkeyseg[second_best]);
        }
        else printf("Ambiguity score: cannot be calculated, no best key or second best key found\n\n");
      }
    }

    if(segtotal==0) { // BETE: If there's only one segment, print it out as the output
      print_keyname(best_key);
      printf("\n");
    }
...
int make_first_table() {
  ...
  double seg_factor, mod_factor, nomod_factor, first_seg_factor;

  if(scoring_mode==3) {
      mod_factor = log(change_penalty);
      nomod_factor = log(1.0 - change_penalty);
      seg_factor = 1.0;
      first_seg_factor = log(1.0 / 24.0);
  }
  ...
  for(i=0; i<56; i++) {
      first_seg_analysis[i] = (key_score[0][i] + first_seg_factor) * seg_factor;
  }

  for(i=0; i<56; i++) {
    for(j=0; j<56; j++) {
      if (j != i) analysis[1][i][j] = first_seg_analysis[i] + (key_score[1][j] * seg_factor) + mod_factor;
      else analysis[1][i][j] = first_seg_analysis[i] + (key_score[1][j] * seg_factor) + nomod_factor;
    }
  }
  choose_best_i();
  return EXIT_SUCCESS;
...
int best_key_analysis() {
    ...
    double mod_factor, nomod_factor;
    ...
    if(verbosity>=2) { /* Print out the segment scores */
      printf("Segment 0: key choice %d; total score = %6.3f; segment tonalness score = %6.3f\n",
             final[0], first_seg_analysis[final[0]], first_seg_analysis[final[0]]);
      printf("Segment 1: key choice %d; total score = %6.3f; segment tonalness score = %6.3f\n",
             final[1], analysis[1][final[0]][final[1]], analysis[1][final[0]][final[1]] - first_seg_analysis[final[0]]);
      for(s=2; s<=segtotal; s++) {
        printf("Segment %d: key choice %d; total score = %6.3f; segment tonalness score = %6.3f\n",
               s, final[s], analysis[s][final[s-1]][final[s]], analysis[s][final[s-1]][final[s]] - analysis[s-1][final[s-2]][final[s-1]]);
      }
    }

    if(verbosity>=2 && scoring_mode < 3) {
      /* This routine calculates "key-fit" scores for the final analysis. These are really per-second scores, in which key-profile scores
      are not multiplied by seglength (as they would be in actually computing the analyses). However, with scoring_mode = 3, scores were
      not multiplied by segment length anyway, so this is not relevant. */
      printf("'Key-fit' scores for preferred analysis:\n");
      for(s=0; s<=segtotal; s++) {
        f=final[s];
        if(s>0 && final[s]!=final[s-1]) keyfit_score[s]=(key_score[s][f]) - (change_penalty / seglength);
        else keyfit_score[s]=key_score[s][f];
      }
